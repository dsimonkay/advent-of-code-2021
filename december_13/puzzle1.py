...
def fold_up(paper, y, rows, cols):
    """
    Folds the paper upwards, mirroring the dots from the lower part to the upper one.
    """
...
# Count the resulting dots visible
nr_of_dots_visible = 0
for i in range(rows):
    for j in range(cols):
        if paper[i][j]:
            nr_of_dots_visible += 1

print(f"\nNumber of dots visible after {NR_OF_FOLDS} folds: {nr_of_dots_visible}")
