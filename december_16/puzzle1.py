...
def parse_packet(input, stack, packets_to_read = 1):
    ...
    while input[ch_count:] and packets_read < packets_to_read:
        ...
        if type_id == TYPE_ID_LITERAL:
            ...
            subpacket_chars = 0
            if length_type_id == "0":
                subpacket_length = int(input[ch_count + 7: ch_count + 22], 2)
                subpacket_chars = parse_packet(input[ch_count + 22: ch_count + 22 + subpacket_length], stack, 999)
                assert subpacket_chars == subpacket_length

            else:
                nr_of_packets_to_read = int(input[ch_count + 7: ch_count + 18], 2)
                subpacket_chars = parse_packet(input[ch_count + 18:], stack, nr_of_packets_to_read)

            ch_count += 7 + subpacket_chars
